 ...
 * - maxWorkers: how many worker processes are allowed to do its task concurrently, default: num of cpus
 * - outFile: the data file name, to be postfixed with worker ID
 * - workerFarm: override worker farm, for testing only
 */
DataGen.prototype.generate = function (opts) {
  ...
  var workerFarmOpts = {};
  if (opts.maxWorkers) {
    workerFarmOpts.maxConcurrentWorkers = opts.maxWorkers;
  }
  
  workerFarm = opts.workerFarm || workerFarm;
